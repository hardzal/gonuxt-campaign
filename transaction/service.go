...
type Service interface {
	...
	ProcessPayment(input NotificationTransactionInput) error
}
...
func (s *service) ProcessPayment(input NotificationTransactionInput) error {
	transaction_id, _ := strconv.Atoi(input.OrderID) // integer to string
	transactionPayment, err := s.repository.GetByID(transaction_id)
	...
	updatedTransaction, err := s.repository.Update(transactionPayment)
	if err != nil {
		return err
	}
